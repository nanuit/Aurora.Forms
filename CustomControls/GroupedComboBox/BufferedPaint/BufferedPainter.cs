...
namespace Aurora.Forms.CustomControls.GroupedComboBox.BufferedPaint
{
    ...
    public class BufferedPainter<TState>
    {
        ...
        public BufferedPainter(Control control)
        {
            ...
            BufferedPaintSupported = (Environment.OSVersion.Version.Major >= 6) && VisualStyleRenderer.IsSupported && Application.RenderWithVisualStyles;
            ...
            if (BufferedPaintSupported && Enabled)
            {
                ...
                if (hdc != IntPtr.Zero)
                {
                    ...
                    if (!Interop.BufferedPaintRenderAnimation(Control.Handle, hdc))
                    {
                        ...
                        IntPtr hbpAnimation = Interop.BeginBufferedAnimation(Control.Handle, hdc, ref rc, BP_BUFFERFORMAT.BPBF_COMPATIBLEBITMAP, IntPtr.Zero, ref animParams, out hdcFrom, out hdcTo);
                        if (hbpAnimation != IntPtr.Zero)
                        {
                            if (hdcFrom != IntPtr.Zero) OnPaintVisualState(new BufferedPaintEventArgs<TState>(_currentState, Graphics.FromHdc(hdcFrom)));
                            if (hdcTo != IntPtr.Zero) OnPaintVisualState(new BufferedPaintEventArgs<TState>(_newState, Graphics.FromHdc(hdcTo)));
    ...
    public class BufferedPaintTransition<TState> : IEquatable<BufferedPaintTransition<TState>>
    {
        ...
        {
            return Equals(this.FromState, other.FromState) && Equals(this.ToState, other.ToState);
        }
    ...
    public class VisualStateTrigger<TState> : IEquatable<VisualStateTrigger<TState>>
    {
        ...
        {
            return (this.Type == other.Type) && Equals(this.State, other.State);
        }
